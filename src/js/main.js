...
export function getStoragePassword() {
...
// // Функция для загрузки страницы без перезагрузки
// function loadPage(route) {
//     // Получаем текущий маршрут
//     const currentRoute = router.getCurrentLocation().url;
//     console.log(currentRoute)
//     // Проверяем, совпадает ли текущий маршрут с маршрутом, который мы хотим загрузить
//     if (currentRoute !== route) {
//         // Используем метод navigate() для перехода на указанный маршрут
//         router.navigate(route);
//     }
// }

// // Вызываем функцию для загрузки главной страницы
// loadPage('/');
...
wrapper.append(load.load, authModal.modal, pageContainer, mobileNav.mobileNav);
...
export function renderItems(memes) {
    
    const itemsList = document.querySelector('.home__list');
    itemsList.innerHTML = '';
    let i = 0;
    // let targetArray = memes.slice(0, toSlice);
    memes.forEach(item => {
...
function itemToHtml(item, index) {
    ...
    const authorLink = createElement('div', 'item__author');
    // authorLink.href = '/';
    ...
    let tagsList = createElement('span', 'item__tags-list', 'visually-hidden');
    
    if (item.tags) {
        item.tags.forEach(el => {
            tagsList.innerText = tagsList.innerText + el.value + ' ';
        })
    }
    ...
    itemActions.append(itemInfo, likeDiv, tagsList);
