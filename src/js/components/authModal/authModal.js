...
import { app, authModal, isAuth, userName, userToken, getInfo, getStorageLogin, getStoragePassword} from "../../main";
...
export function getAuthModal() {
    ...
    const logoBox = createElement('div', 'modal__logo-box');
    const logoText = createElement('p', 'modal__logo-box--text');
    logoText.innerText = 'Социальная сеть мемов';
    const logo = getLogo();
    logo.classList.add('auth__logo')
    logoBox.append(logo, logoText);
    ...
    loginInput.value = getStorageLogin();
    ...
    passwordInput.value = getStoragePassword();
    ...
    emailRegistrInput.required = 'required';
    ...
    loginRegistrInput.required = 'required';
    ...
    modalBox.append(logoBox, modalItem, modalClose, modalItem2, modalForm);
    ...
    let registerUser = (event) => {
        ...
        createUserWithEmailAndPassword(auth, emailRegistrInput.value, passwordRegistrInput.value).then((credentials) => {
            ...
            loginInput.value = emailRegistrInput.value;
            passwordInput.value = passwordRegistrInput.value;
